...
from training.wrappers import SkipFrameWrapper, FrameDownsampleWrapper
...
class Mario(Mario):
    def __init__(self, state_dim, action_dim, save_dir):
        ...
        self.beta_end = 1.0
        self.beta_anneal_steps = 2000000
        self.step_count = 0
        self.beta_start = 0.4
        self.gamma = 0.9
    ...
    def cache(self, state, next_state, action, reward, done):
        ...
        self.nstep_buffer.append(TensorDict({"state": state, "next_state": next_state,
                                     "action": action,"reward": reward, "done": done,
                                       "td_error": self.max_priority}, batch_size=[]))
        ...
        if len(self.nstep_buffer) == 4:
            ...
            first["reward"]     = torch.as_tensor([G],     dtype=torch.float32,    device=self.device)
    ...
    def recall(self):
        ...
        #    raise KeyError("Next state is None. Check your memory sampling.")

        is_weights = info["_weight"].to(self.device)
        index = info["index"]
        if is_weights is None or index is None:
            print(info.keys())
            raise KeyError("IS weights and indices are None. Check your memory sampling.")
